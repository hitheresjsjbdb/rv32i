...
wire done_cu;
wire IDUseRs1, IDUseRs2, IDIsLoad, IDIsStore, IDIsBranch, IDIsJal, IDIsJalr, IDIsEbreak;
reg done_r;
assign done = done_r;
...
assign Offset  = IDIsBranch ? {out_ins[31], out_ins[7], out_ins[30:25], out_ins[11:8]} :
                 IDIsStore  ? {out_ins[31:25], out_ins[11:7]} : Imm12;
...
wire id_use_rs1_eff;
wire id_use_rs2_eff;
assign id_use_rs1 = IDUseRs1;
assign id_use_rs2 = IDUseRs2;
assign id_use_rs1_eff = ifid_valid && id_use_rs1;
assign id_use_rs2_eff = ifid_valid && id_use_rs2;

wire load_use_hazard_rs1;
wire load_use_hazard_rs2;
assign load_use_hazard_rs1 = id_use_rs1_eff && idex_valid && idex_MemRead && (idex_rd != 5'd0) && (idex_rd == rs1);
assign load_use_hazard_rs2 = id_use_rs2_eff && idex_valid && idex_MemRead && (idex_rd != 5'd0) && (idex_rd == rs2);
...
assign id_rs1_val_byp = (id_use_rs1_eff && memwb_can_fwd && (memwb_rd == rs1)) ? memwb_fwd_data : RD1;
assign id_rs2_val_byp = (id_use_rs2_eff && memwb_can_fwd && (memwb_rd == rs2)) ? memwb_fwd_data : RD2;

assign ex_rs1_fwd = (idex_valid && idex_UseRs1) ? ((exmem_can_fwd && (exmem_rd == idex_rs1)) ? exmem_fwd_data :
                                                   (memwb_can_fwd && (memwb_rd == idex_rs1)) ? memwb_fwd_data :
                                                                                                idex_rs1_val)
                                                : 32'b0;
assign ex_rs2_fwd = (idex_valid && idex_UseRs2) ? ((exmem_can_fwd && (exmem_rd == idex_rs2)) ? exmem_fwd_data :
...
assign ex_branch_target = idex_valid ? (idex_IsJalr ? ((ex_rs1_fwd + idex_npc_imm) & 32'hffff_fffe)
                                                     : (idex_pc + idex_npc_imm))
                                    : 32'b0;
assign ex_recover_npc = ex_take_branch ? ex_branch_target : (idex_valid ? (idex_pc + 32'd4) : 32'b0);
assign ex_dnpc = ex_recover_npc;
assign ex_mispredict = idex_valid ? (ex_take_branch ^ idex_pred_taken) : 1'b0;
...
assign bp_update_pc = bp_update_valid ? idex_pc : 32'b0;
assign bp_update_taken = bp_update_valid ? ex_branch_cond : 1'b0;
assign bp_report_valid = memwb_valid && memwb_IsEbreak;
wire bp_update_write_en;
assign bp_update_write_en = bp_update_valid && (bp_bht[bp_update_idx] != bp_update_taken);
...
assign A_eff = idex_valid ? A : 32'b0;
assign B_eff = idex_valid ? B : 32'b0;
assign ALUOp_eff = idex_valid ? idex_ALUOp : `ALUOp_ADD;
assign dm_access_en = exmem_valid && (exmem_MemRead || exmem_MemWrite);
assign dm_addr_eff = dm_access_en ? exmem_alu_result[11:2] : 10'b0;
assign dm_wd_eff = (exmem_valid && exmem_MemWrite) ? exmem_rs2_val : 32'b0;
...
ControlUnit U_ControlUnit(
    ...
    .IDUseRs1(IDUseRs1), .IDUseRs2(IDUseRs2), .IDIsLoad(IDIsLoad), .IDIsStore(IDIsStore),
    .IDIsBranch(IDIsBranch), .IDIsJal(IDIsJal), .IDIsJalr(IDIsJalr), .IDIsEbreak(IDIsEbreak)
);
...
ALU U_ALU (
    .A(A_eff), .B(B_eff), .ALUOp(ALUOp_eff), .ALU_result(ALU_result), .zero(zero)
);
...
DM U_DM (
    .Addr(dm_addr_eff), .WD(dm_wd_eff), .DMCtrl(DMCtrl_eff), .clk(clk), .RD(RD)
);
    ...
    if (rst) begin
        ...
        if (ex_can_advance) begin
            if (!ifid_valid || id_stall || ex_branch_commit) begin
                ...
                idex_npc_imm <= IDIsBranch ? ImmB32 :
                                IDIsJal    ? ImmJ32 :
                                             Imm32;
                idex_rd <= rd;
                idex_rs1 <= rs1;
                ...
                idex_MemRead <= IDIsLoad;
                idex_MemWrite <= IDIsStore;
                idex_IsBranch <= IDIsBranch;
                idex_IsJal <= IDIsJal;
                idex_IsJalr <= IDIsJalr;
                idex_IsEbreak <= IDIsEbreak && (rs1 == 5'd0) && (rs2 == 5'd1) && (rd == 5'd0);
                idex_pred_taken <= ifid_pred_taken;
                idex_UseRs1 <= id_use_rs1;
                idex_UseRs2 <= id_use_rs2;
            end
        end
