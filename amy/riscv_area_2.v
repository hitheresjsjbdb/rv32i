...
wire done_cu;
wire DecUseRs1, DecUseRs2;
wire DecIsLoad, DecIsStore, DecIsBranch, DecIsJal, DecIsJalr;
reg done_r;
assign done = done_r;
...
localparam BP_BHT_ENTRIES = 4;
localparam BP_BHT_IDX_W = 2;
reg [BP_BHT_ENTRIES-1:0] bp_bht;
...
localparam [1:0] FLOW_NONE   = 2'b00;
localparam [1:0] FLOW_BRANCH = 2'b01;
localparam [1:0] FLOW_JAL    = 2'b10;
localparam [1:0] FLOW_JALR   = 2'b11;
...
assign Imm12   = DecIsStore ? {out_ins[31:25], out_ins[11:7]} : out_ins[31:20];
...
wire id_use_rs1;
wire id_use_rs2;
assign id_use_rs1 = DecUseRs1;
assign id_use_rs2 = DecUseRs2;
...
wire ex_mispredict;
assign ex_take_branch  = idex_valid && ((idex_Flow == FLOW_BRANCH) ? ex_branch_cond : (idex_Flow != FLOW_NONE));
// For JALR, branch target must use EX-forwarded rs1 to avoid RAW hazards.
assign ex_branch_target = (idex_Flow == FLOW_JALR) ? ((ex_rs1_fwd + idex_npc_imm) & 32'hffff_fffe)
...
assign bp_update_valid = ex_can_advance && idex_valid && (idex_Flow == FLOW_BRANCH);
...
ControlUnit U_ControlUnit(
    ...
    .DecUseRs1(DecUseRs1), .DecUseRs2(DecUseRs2),
    .DecIsLoad(DecIsLoad), .DecIsStore(DecIsStore), .DecIsBranch(DecIsBranch),
    .DecIsJal(DecIsJal), .DecIsJalr(DecIsJalr)
);
...
MUX_3to1_B U_MUX_3to1_B (
    .X(ex_rs2_fwd), .Y(idex_imm32), .Z(12'b0), .control(idex_ALUSrcB), .out(B)
);
    ...
    if (rst) begin
        ...
        if (ex_can_advance) begin
            if (!ifid_valid || id_stall || ex_branch_commit) begin
                ...
                idex_imm32 <= Imm32;
                idex_npc_imm <= DecIsBranch ? ImmB32 :
                                DecIsJal    ? ImmJ32 :
                                              Imm32;
                idex_rd <= rd;
                idex_rs1 <= rs1;
                ...
                idex_MemRead <= DecIsLoad;
                idex_MemWrite <= DecIsStore;
                idex_Flow <= DecIsBranch ? FLOW_BRANCH :
                             DecIsJal    ? FLOW_JAL :
                             DecIsJalr   ? FLOW_JALR :
                                           FLOW_NONE;
